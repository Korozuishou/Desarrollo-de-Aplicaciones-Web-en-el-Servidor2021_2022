...
function longitud($contenido){
	$resultadofun = strlen($contenido);
	return $resultadofun;
}

function existeono($i,$contenido,$entrada){
//	$resultado = strcmp($entrada,$contenido);
//	return $resultado;
	$longitud = strlen($contenido);

	while ($i <= $longitud && $contenido[$i]<>$entrada){
		$i=$i+1;
	}
	if ($i > $longitud){
		return 0;
